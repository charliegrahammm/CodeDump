# +----------------------------------+----------------------------------------------------------+
# |   ___    ____    _   _   _       | Title:   check_lcms_foldersync.ps1                        |
...
# +----------------------------------+----------------------------------------------------------+
# | DESCRIPTION:                                                                                |
# | Script for Nagios. Checks the status of the LC-MS FolderSync using S3 Browser                 |
# |                                                                                             |
...
$recentS3Sync = "C:\Jobs\LC-MS-FolderSync\recent.txt"
...
Try {
	...
	if ($syncCompletedCheck -eq $null) {
		write-host "CRITICAL: Sync not completed yet, likely that the job is still running."
		Exit 2
	} elseif ($errorsS3Reported -eq $null) {
		write-host "Sync completed at $syncCompletedTime. Files $syncCompletedValues"
		Exit 0
	} else {
		write-host "S3 ran into the following errors:`n`n" ($errorsS3Reported -join "`n`n")
		Exit 1
	}

} Catch {
	write-host "Script ran into an error.$($_.Exception) at line $($_.InvocationInfo.ScriptLineNumber)"
    Exit 1
}
}
}
