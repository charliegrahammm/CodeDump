# +----------------------------------+---------------------------------------------+
# |   ___    ____    _   _   _       | Title:   FireAndForgetWindowsUpdates.ps1    |
...
# +----------------------------------+---------------------------------------------+
# | DESCRIPTION:                                                                   |
# | This keeps running and installing Windows 10 updates and rebooting, until all  |
# | available updates are installed.                                               |
# |                                                                                |
# | Checks for updates. Using PSWindowsUpdate.                                     | 
# | You must have previously run InstallPSWindowsUpdate.ps1                        |
# |                                                                                |
# | If updates are found, the computer is set to automatically login as LocalUser  |
# | and this script is set to run again at logon.                                  |
# |                                                                                |
# | If no updates are found, the automatic login and script are removed.           |
# |                                                                                |
# | Uses this:                                                                     |
...
Try {
	...
	If (Get-WUInstall –MicrosoftUpdate) {
		...
		Write-Host "Installing Ivanti Patch Agent..."
#		& "\\bandini\share\Setup\Build Scripts\Ivanti\Ivanti.cmd"
		. "\\bandini\share\Setup\Build Scripts\Ivanti\Ivanti.ps1" 
		InstallIvanti -Policy "CloudAgent" -Passphrase "!Architecture#Screwdriver24!"
		Sleep 10
		...
		If (Test-Path "$($env:TEMP)\Build_Computername.txt") {
			$ComputerName = Get-Content "$($env:TEMP)\Build_Computername.txt"
			$ComputerName = $ComputerName.Trim()
			
			# If we don't already have stored credentials
			If (Test-Path "$($env:TEMP)\DomainJoinCredentials.xml") {
				Write-Host "Stored Domain Joining credentials were found"
				# Read in the user credentials from the file
				$DomainJoinCredentials = Import-CliXml "$($env:TEMP)\DomainJoinCredentials.xml"
				
				Try {
					$NoDisplay = Rename-Computer -NewName $ComputerName
					Start-Sleep 20
					Add-Computer –DomainName "PHARMAXO.local" -Credential $DomainJoinCredentials –Force -Options JoinWithNewName,AccountCreate -Restart
				} Catch {
					Write-Host -ForegroundColor Red "Error '$($_.Exception.Message)'"
					Write-Host "You will have to Domain Join Manually."
					Write-Host "Press a key to continue..."
					$Null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
				}    

			} Else {
				Write-Host "No stored Domain Joining credentials found. You'll have to perform this step manually."
				Write-Host "Press a key to continue..."
				$Null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
			}
		} Else {
			Write-Host -ForegroundColor Red "No Computer Name Stored for Domain Join. You'll have to do this manually."
			Write-Host "Press a key to continue..."
			$Null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
		}
